<?php
//Okej więc całość działa tak,że pierw zaciąga search API
//Następnie z search API wyciąga dane na temat kluczy które pozwalają 
//Wykorzystać books API gdzie są już opisy, tytuły w zdatnej formie
//NASTĘPNIE z books API wyciągamy numer okładki (jednej) by zaciągnąć z Covers API obrazek.
//Po drodze też są konwersje z jsona na arraya.
if(isset($_POST['szukane']))
{    
  //Curl do API OpenLibrary (Search)
  $szukanyTyp ="author";
  $szukaneHaslo ="Harry potter"; 
  $adresKsiazki = null;
  $hasloDoLinku = str_replace(" ", "+", $szukaneHaslo);
    
  $szukaneCalosc = "http://openlibrary.org/search.json?title=$hasloDoLinku"; 
  $ch = curl_init($szukaneCalosc);

  $fp = fopen("wygenerowane/searchResults.json", "w");
...
$szukane_klucze_json = file_get_contents("wygenerowane/searchResults.json"); 
$szukane_klucze_array = json_decode($szukane_klucze_json, true);
   
$sprawdzone = $szukane_klucze_array['docs'];
    
  //Okej więc to jest bardzo at work. Muszę przepuścić to przez jeszcze jedno API 
  //z wykorzystaniem klucza który dostałam z tego api :') Inaczej dostaję głównie bezużyteczne rzeczy z tego API
  ...
  for($i = 0; $i<5; $i++)
  {
    ...
    $fpBooks = fopen("wygenerowane/bookKeys.json", "w");
    ...
    $szukane_ksiazki_json = file_get_contents("wygenerowane/bookKeys.json");
